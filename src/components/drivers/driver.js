...
export default function DriversPage() {
  ...
  function showConfirm(id) {
    confirm({
      title: `Do you Want to delete ${id} Driver ?`,
  ...
  const columns = [
    ...
    { title: 'Approve', dataIndex: 'approved', key: 'approved' },
  ...
  const data = dispatcher && dispatcher.map(el => {
    return {
      name: el.username,
      email: el.email,
      phoneNo: `+1${el.phoneNo}`,
      approved: <Switch style={{background: el.approve == true ? "green": "red"}} defaultChecked={el.approve} onChange={(e) => onChange(el)} />,
      edit: <Button onClick={() => {
        seteditCutomer(el)
