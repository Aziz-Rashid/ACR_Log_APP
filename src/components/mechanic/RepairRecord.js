...
import { Table, Alert, Menu, Dropdown, Modal, Input } from 'antd'
...
const columns = [
    ...
    { title: 'Trailer Make', dataIndex: 'reefer', key: 'reefer' },
...
export default function Row({ status }) {
    const { confirm } = Modal;
    ...
    const filtertrailer = mechanic?.filter(el => el.vehicleType == "Trailer")

    let newarr = filtertrailer?.map(el => {
                                    ...
                                    <>
                                        {item.user != user.email ? (
                                            <div className="aswass">
                                                <h1 className="hahahaha">{item.value}<span className="wqaa">{new Date(item.time.seconds * 1000).toLocaleString()}</span></h1>
                                            </div>
                                        ) : (
                                                <>
                                                    {item.user == user.email ? (
                                                        <div className="aswassss">
                                                            <h1 className="hahahaha">{item.value}<span className="wqaas">{new Date(item.time.seconds * 1000).toLocaleString()} <DeleteOutlined onClick={() => deletenotes(item)} /></span></h1>
                                                        </div>
                                                    ) : null}
                                                </>
                                            )}
