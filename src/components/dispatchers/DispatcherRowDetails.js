...
export default function FullWidthTabs({row, editCallback}) {
    ...
    const handleSave = () => {
        firebase.firestore().collection('dispatchers').doc(fnstateObj.name).set(fnstateObj).then(
            ()=>{
                auth.createUserWithEmailAndPassword(fnstateObj.email, fnstateObj.password).then((user) => {
                    if (user) {
                        router.replace('/')
                        auth.signOut()
                    } else {
...
function Details({row, edit, callback}) {
    ...
    const getMap = (stateObj) => Object.keys(stateObj).map(key => {
        if (key != 'history' && key != "confirmpassword" && key != "verified" && key != "accessLevel" ) {
            return (
                <ListItem key={key}>
